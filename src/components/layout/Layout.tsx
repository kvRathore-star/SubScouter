  ...
  return (
    <div className="flex h-screen w-full bg-background text-foreground font-sans selection:bg-brand/10">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:ml-0 overflow-hidden relative">
        ...
        <div className="w-full max-w-[1200px] mx-auto px-6 sm:px-8 lg:px-12 py-6 flex items-center justify-between z-10">
          <div className="flex items-center gap-4">
            {/* Desktop Menu Toggle */}
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div
                onClick={() => setView('settings')}
                className="flex items-center gap-3 cursor-pointer group hover:bg-secondary/30 p-1.5 pr-4 rounded-2xl border border-transparent hover:border-border transition-all duration-300"
              >
                <div className="w-9 h-9 rounded-xl overflow-hidden border border-border group-hover:border-brand/40 transition-all shadow-md relative">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth">
          <main className="px-6 sm:px-8 lg:px-12 pt-8 lg:pt-12 pb-12 w-full max-w-[1200px] mx-auto min-h-full">
            <ErrorBoundary>
              {children}
            </ErrorBoundary>

            {/* Trust Footer */}
            <div className="mt-20 pt-8 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-6 opacity-40 hover:opacity-100 transition-opacity duration-700">
              <div className="flex items-center gap-2">
                <Shield className="w-4 h-4 text-brand" />
                <span className="text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-widest">Security & Privacy Protocol</span>
              </div>
              <div className="text-[10px] text-muted-foreground text-center sm:text-right max-w-sm">
                Gmail intelligence is scanned via secure Edge AI logic. Data remains isolated and is never sold to third parties.
              </div>
            </div>
          </main>
        </div>
